...
class MyLightningModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        self.lr_scheduler = optim.lr_scheduler.StepLR(optimizer, 200, gamma=0.999)
        return optimizer
...
def train():
    wandb.init()
    config = wandb.config
    # config.update("num_of_features", int(config.data_path.split("/")[-1].split("_")[1]))
    ...
    sweep_config = {
        ...
        'parameters': {
            ...
            'weight_decay': {'distribution': "uniform", 
                              'max': 8e-7, 
                              'min': 6e-7},
            # 'learning_rate': {'value': 8e-5},
            # 'weight_decay': {'value': 1e-7},
            "data_path": {'value': "/import/c4dm-04/jt004/ATEPP-data-exp/processed_data/id_13_segment_4.npz"},
            "ckpt_path": {'value': args.ckpt_path},
            "save_path": {'value': args.save_path},
            "model": {'value': "PID_CNN1D"},
            "num_of_performers": {'value': args.num_of_performers},
            "num_of_features":{'value' : args.num_of_features},
            "kernal_size": {'values': [[3, 5, 5, 3],
                                       [3, 5, 5, 5],
                                       [5, 5, 5, 3], 
                                       [7, 5, 5, 3],
                                       [5, 5, 5, 5], 
                                       [7, 5, 5, 5]]},
            "dropout": {'value': 0.5},
            "dense_size": {'value': 512},
            "times": {'values': [0, 1, 2]}
        }
    }

    sweep_id=wandb.sweep(sweep_config, project="PID")
